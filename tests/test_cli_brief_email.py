...
def test_email_does_not_build_pdf_charts_or_call_plan_coach(runner, wired, monkeypatch):
    from local_fitness.agent import tools as agent_tools

    async def unexpected(*args, **kwargs):
        raise AssertionError("The digest must not render charts or generate coaching")

    monkeypatch.setattr(agent_tools, "assemble_brief_render_inputs", unexpected)
    result = runner.invoke(cli.main, ["brief-email", "--no-pull", "--no-generate"])
    assert result.exit_code == 0, result.output
    msg = wired["last_msg"]
    assert [p.get_content_type() for p in msg.walk()] == [
        "multipart/alternative", "text/plain", "text/html"]
    assert "Plan unavailable" in msg.get_body(preferencelist=("plain",)).get_content()


def test_mismatched_saved_brief_date_does_not_mix_days_or_send(runner, wired):
    (wired['dir'] / f'{TODAY}.json').write_text(BRIEF_JSON % ('2000-01-01', '2000-01-01'))
    result = runner.invoke(cli.main, ['brief-email', '--no-pull', '--no-generate'])
    assert result.exit_code == 1
    assert 'date does not match' in result.output
    assert wired['send'] == 0
    assert not marker(wired).exists()
